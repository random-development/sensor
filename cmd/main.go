...
func main() {
	...
	cpuCollector := internal.MakeCollector(internal.CPUProbe{}, psBroker)
	cpuCollector.Run(2 * time.Second)

	// spawn goroutines for consuming measurements generated by collectors
	go consumeMeasurements(psBroker.Sub(internal.MemProbe{}.Resource()))
	go consumeMeasurements(psBroker.Sub(internal.CPUProbe{}.Resource()))
	// start from here
	// publisher := MakeWebSocketPublisher("cpu", "anything",
	// handle interrupt
